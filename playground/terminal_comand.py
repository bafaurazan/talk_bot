...
async def on_message(message):
    ...
    if message.content.startswith('$k2'):
        process = subprocess.run(['C:/Program Files/Git/git-bash','-c','C:/maxima-5.45.1/bin/maxima.bat'], capture_output=True)

    if message.content.startswith('$k3'):
        process = subprocess.run(['C:/Program Files/Git/git-bash','-c', 'nano'], capture_output=True)
